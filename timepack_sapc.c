...
int starttimer()
{
  ...
  current = inpt(TIMER0_COUNT_PORT);
  current |= (inpt(TIMER0_COUNT_PORT)<<8);
  current = COUNTS_PER_TICK - current;
  startcount = (double)(tickcount * COUNTS_PER_TICK);
  startcount += current;
  timer_running = TRUE;
  return T_OK;
}
...
int stoptimer( int *interval )
{
  ...
  tickcount += current;
  *interval = (int)((double)(tickcount - startcount) * (USECS_PER_TICK / COUNTS_PER_TICK));
#ifdef DEBUG
